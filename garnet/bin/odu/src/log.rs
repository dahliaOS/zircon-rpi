...
impl Stats {
    ...
    /// thread for duration or the run.
    ...
    fn stage_stats(&self, stage: PipelineStages) -> (f64, f64, f64) {
        let mut duration = self.stage_duration[stage.stage_number()] as f64 / 1_000_000_000 as f64;
        let d = self.stage_duration[stage.stage_number()];

        if duration == 0.0 {
            warn!("Stage duration is zero for {:?}", stage);
            duration = 1.0;
        }
        let latency_us = (d as f64 / 1_000 as f64) / self.io_count as f64;
        let write_bandwidth = (self.write_size / 1_000_000) as f64 / (duration as f64);
        let iops_per_sec = self.io_count as f64 / (duration as f64);
        (latency_us, write_bandwidth, iops_per_sec)
    }

    fn display_summary_per_stage(&self) {
        for stage in PipelineStages::iterator() {
            let threads_duration = self.stage_duration[stage.stage_number()].clone();
            let (latency_us, write_bandwidth, iops_per_sec) = self.stage_stats(stage.clone());
            debug!(
    ...
    pub fn display_summary(&self) {
        ...
        if log_enabled!(Debug) {
            self.display_summary_per_stage();
        }
        println!(
