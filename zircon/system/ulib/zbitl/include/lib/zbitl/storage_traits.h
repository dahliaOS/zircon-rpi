...
struct StorageTraits {
  ...
  /// The container must within this maximum.  Storage past the container's
  ...
  static std::variant<error_type, uint32_t> Capacity(Storage& zbi) { return {}; }
  ...
  static std::variant<error_type, zbi_header_t> Header(Storage& zbi, uint32_t offset) { return {}; }
  ...
  static std::variant<error_type, payload_type> Payload(Storage& zbi, uint32_t offset,
                                                        uint32_t length) {
    return {};
  }
  ...
  static std::variant<error_type, uint32_t> Crc32(Storage& zbi, uint32_t offset, uint32_t length) {
    return {};
  }
};
...
struct StorageTraits<std::basic_string_view<T>> {
  ...
  static std::variant<error_type, uint32_t> Capacity(Storage& zbi) {
    return static_cast<uint32_t>(std::min(zbi.size(), static_cast<typename Storage::size_type>(
                                                          std::numeric_limits<uint32_t>::max())));
  }

  static std::variant<error_type, std::reference_wrapper<const zbi_header_t>> Header(
  ...
  static std::variant<error_type, uint32_t> Crc32(Storage& zbi, uint32_t offset, uint32_t length) {
    auto payload = zbi.substr(offset, length);
    ZX_DEBUG_ASSERT(payload.size() == length);
    return {std::in_place_index<1>,
            crc32(0, reinterpret_cast<const uint8_t*>(payload.data()), payload.size())};
  }
};

// Specialization for std::span<any type> as Storage.  Its payload_type is the
// same type as Storage, just yielding the subspan of original whole-ZBI span.
#if __cpp_lib_span
...
struct StorageTraits<std::span<T, Extent>> {
  ...
  static std::variant<error_type, payload_type> Payload(Storage& zbi, uint32_t offset,
    ...
    return payload_type{reinterpret_cast<T*>(payload.data()), payload.size() / sizeof(T)};
  }
