...
void JsonWriteHeader(Writer&& writer, const zbi_header_t& header,
  ...
  if (TypeIsStorage(header) && (header.flags & ZBI_FLAG_STORAGE_COMPRESSED)) {
    ...
    uint32_t expected_extra = header.type == ZBI_TYPE_CONTAINER ? ZBI_CONTAINER_MAGIC : 0;
...
void JsonWriteZbi(Writer&& writer, Zbi&& zbi, std::optional<uint32_t> offset = {},
  ...
  auto advance_offset = [&offset](uint32_t payload_length) {
    if (offset) {
      *offset += sizeof(zbi_header_t) + ZBI_ALIGN(payload_length);
    }
  };

  if (auto container = zbi.container_header()) {
    writer.StartObject();

    JsonWriteHeader(writer, *container, offset);
    advance_offset(0);
