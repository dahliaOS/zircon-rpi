...
void ComponentProxy::I2cTransact(const i2c_op_t* ops, size_t cnt, i2c_transact_callback transact_cb,
                              void* cookie) {
    ...
    size_t req_length = sizeof(I2cProxyRequest) + cnt * sizeof(I2cRpcOp) + writes_length;
    ...
    auto rpc_ops = reinterpret_cast<I2cRpcOp*>(req + 1);
    ...
    uint8_t* p_writes = reinterpret_cast<uint8_t*>(rpc_ops) + cnt * sizeof(I2cRpcOp);
    for (size_t i = 0; i < cnt; ++i) {
        if (!ops[i].is_read) {
    ...
    I2cProxyResponse* rsp = reinterpret_cast<I2cProxyResponse*>(resp_buffer);
    size_t actual;
