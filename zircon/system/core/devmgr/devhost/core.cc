...
zx_status_t devhost_device_get_dev_state_from_suspend_flags(
  ...
  // In addition to translating them to system power states, we might
  // need to provide an extra hint or context to the state, for cases
  // like REBOOT/REBOOT_BOOTLOADER/REBOOT_RECOVERY/MEXEC. For now, each of these
  // flags are treated as an individual state.
...
zx_status_t devhost_device_suspend(const fbl::RefPtr<zx_device>& dev, uint32_t flags) REQ_DM_LOCK {
  ...
  if (dev->ops->suspend_new) {
    ...
    ApiAutoRelock relock;
    status = devhost_device_get_dev_state_from_suspend_flags(dev, flags, &new_state_info);
    if (status == ZX_OK) {
      status = dev->ops->suspend_new(dev->ctx, new_state_info.dev_state, new_state_info.wakeup_enable,
                            &out_state);
    }
