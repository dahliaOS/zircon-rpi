...
TEST_F(MsiTest, AllocationAndCreation) {
  ...
  // Ensure the check for the capability ID is valid
  ASSERT_NOT_OK(zx::msi::create(msi, 0, vmo, 0, ZX_INTERRUPT_VIRTUAL, &interrupt));
  ...
  *mock_ctrl_reg = mock_ctrl_val;
  ASSERT_OK(zx::msi::create(msi, 0, vmo, 0, ZX_INTERRUPT_VIRTUAL, &interrupt));
