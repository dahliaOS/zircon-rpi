...
TEST(SimpleAudioTest, Enumerate2) {
  struct MockSimpleAudioLocal : public MockSimpleAudio {
    ...
    zx_status_t Init() __TA_REQUIRES(domain_token()) override {
      ...
      range2.flags = ASF_RANGE_FLAG_FPS_44100_FAMILY;
...
TEST(SimpleAudioTest, WatchPositionAndCloseRingBufferBeforeReply) {
  ...
  constexpr uint32_t kNumberOfPositionNotifications = 1;
  // Buffer is set to hold 1 second, with 10 x kNumberOfPositionNotifications notifications
  // per ring buffer (i.e. per second) the time waiting before getting a position reply is 1 sec.
