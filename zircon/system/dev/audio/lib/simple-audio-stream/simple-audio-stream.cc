...
void SimpleAudioStream::GetChannel(GetChannelCompleter::Sync completer) {
  ...
  fidl::OnUnboundFn<audio_fidl::StreamConfig::Interface> on_unbound =
      [this, stream_channel](audio_fidl::StreamConfig::Interface* server,
                             fidl::UnboundReason reason, zx::channel channel) {
        ScopedToken t(domain_token());
        fbl::AutoLock channel_lock(&channel_lock_);
...
void SimpleAudioStream::CreateRingBuffer(
  ...
  if (!frame_size_) {
    zxlogf(ERROR, "Failed to compute frame size (ch %hu fmt 0x%08x)\n",
           format_v1.number_of_channels, sample_format);
    completer.Close(ZX_ERR_INVALID_ARGS);
    return;
  }
...
void SimpleAudioStream::GetSupportedFormats(
  ...
  for (auto& i : fidl_compatible_formats) {
    ...
    formats.number_of_channels = ::fidl::VectorView<uint8_t>(fidl::unowned_ptr(i.number_of_channels.data()), i.number_of_channels.size());
    formats.sample_formats = ::fidl::VectorView<audio_fidl::SampleFormat>(fidl::unowned_ptr(i.sample_formats.data()), i.sample_formats.size());
    formats.frame_rates = ::fidl::VectorView<uint32_t>(fidl::unowned_ptr(i.frame_rates.data()), i.frame_rates.size());
    formats.bytes_per_sample = ::fidl::VectorView<uint8_t>(fidl::unowned_ptr(i.bytes_per_sample.data()), i.bytes_per_sample.size());
    formats.valid_bits_per_sample = ::fidl::VectorView<uint8_t>(fidl::unowned_ptr(i.valid_bits_per_sample.data()), i.valid_bits_per_sample.size());
  ...
  completer.Reply(::fidl::VectorView<audio_fidl::SupportedFormats>(fidl::unowned_ptr(fidl_formats.data()),
                  fidl_formats.size()));
}
