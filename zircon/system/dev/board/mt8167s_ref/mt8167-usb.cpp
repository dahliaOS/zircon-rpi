...
zx_status_t Mt8167::UsbInit() {
    ...
    constexpr size_t configSize = sizeof(UsbConfig) + 2 * sizeof(FunctionDescriptor);
    UsbConfig* config = reinterpret_cast<UsbConfig*>(
        aligned_alloc(alignment, ROUNDUP(configSize, alignment)));
    if (!config) {
    ...
    usb_metadata[0].data_size = configSize;
