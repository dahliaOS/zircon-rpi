...
TEST(ClockGateTest, TestGateParent) {
  ...
  ClockTree tree(clocks, kClkCount);
...
TEST(ClockGateTest, TestGateUnsupported) {
  ...
  // gating/ungating.
  const uint32_t kChild = 0;
  const uint32_t kMiddle = 1;
  const uint32_t kRoot = 2; 
...
TEST(ClockGateTest, TestExtraneousEnableDisable) {
    // Make sure that calling Enable/Disable multiple times on a clock that is
    // already enabled or disabled does not actually call Enable/Disable on the
    // hardware.
    ...
    // Make sure this logic works in the disable direction as well.
...
TEST(ClockMuxTest, TestReparentTrivial) {
  ...
  const uint32_t parents[] = { kClkFirstParent, kClkSecondParent };
...
TEST(ClockMuxTest, TestReparentEnableDisable) {
  ...
  const uint32_t kClkChild = 0;
  const uint32_t kClkFirstParent = 1;
  const uint32_t kClkSecondParent = 2;
  const uint32_t kClkCount = 3;
  ...
  // Validate the state of the clock tree.
  bool is_enabled = false;
  EXPECT_OK(tree.IsEnabled(kClkChild, &is_enabled));
  EXPECT_TRUE(is_enabled);
  ...
  // Verify the new state of the clock tree.
  is_enabled = false;
  EXPECT_OK(tree.IsEnabled(kClkChild, &is_enabled));
  EXPECT_TRUE(is_enabled);
