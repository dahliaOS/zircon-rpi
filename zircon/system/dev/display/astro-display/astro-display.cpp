...
zx_status_t AstroDisplay::DisplayControllerImplCaptureDisplayOutput(zx::vmo* out_vmo) {
    ...
    DISP_INFO("canvas: index = %d, height = %d, stride = %d\n",
              canvas_idx, image_info->image_height, image_info->image_stride);


    // Start the capture
    return vpu_->Capture(canvas_idx, image_info->image_height, image_info->image_stride);
}
