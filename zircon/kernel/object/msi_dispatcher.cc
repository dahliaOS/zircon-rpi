...
zx_status_t MsiDispatcher::Create(fbl::RefPtr<MsiAllocation> alloc, uint32_t msi_id,
  ...
  if (!out_rights || !out_interrupt || (vmo->is_paged() && !vmo->is_contiguous()) ||
      reg_offset >= vmo->size() || options & ~ZX_INTERRUPT_VIRTUAL ||
      vmo->GetMappingCachePolicy() != ZX_CACHE_POLICY_UNCACHED_DEVICE) {
    LTRACEF(
        "out_rights = %p, out_interrupt = %p\nis_paged = %d, is_contiguous = %d\nsize = %lu, "
        "reg_offset = %lu, options = %#x\n",
        out_rights, out_interrupt, vmo->is_paged(), vmo->is_contiguous(), vmo->size(), reg_offset,
        options);
  ...
  LTRACEF("Mapping mapped at %#lx, size %zx, vmo size %lx, reg_offset = %#lx\n", mapping->base(),
          mapping->size(), vmo->size(), reg_offset);
  fbl::AllocChecker ac;
