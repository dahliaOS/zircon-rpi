...
void PhysMain(void* zbi, arch::EarlyTicks ticks) {
  ...
  // Scan through the ZBI looking for items that configure the serial
  // console.  Note that as each item is encountered, it resets uart to
  // the appropriate variant and sets its configuration values.  So a
  // later item will override the selection and configuration of an
  // earlier item, but this all happens before anything touches hardware.
  using Zbi = zbitl::View<ktl::span<ktl::byte>, zbitl::Checking::kPermissive>;
  auto find_uart = [&uart](Zbi::storage_type blob) {
    Zbi zbi(blob);
    for (auto [header, payload] : zbi) {
      uart.Match(*header, payload.data());
    }
    // Don't bother with any errors reading the ZBI.  Either the console
    // got set up or it didn't.  If the program cares about the ZBI being
    // valid, it will scan it again.
    zbi.ignore_error();
  };
  ...
  find_uart({reinterpret_cast<ktl::byte*>(zbi), UINT32_MAX});
