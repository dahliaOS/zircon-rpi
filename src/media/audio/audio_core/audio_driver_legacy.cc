...
AudioDriverLegacy::AudioDriverLegacy(AudioDevice* owner)
    : AudioDriverLegacy(owner, LogMissedCommandDeadline) {}

AudioDriverLegacy::AudioDriverLegacy(AudioDevice* owner, DriverTimeoutHandler timeout_handler)
    : owner_(owner),
...
zx_status_t AudioDriverLegacy::Init(zx::channel stream_channel) {
  TRACE_DURATION("audio", "AudioDriverLegacy::Init");
  ...
  if (res != ZX_OK) {
    FX_PLOGS(ERROR, res) << "Failed to wait on stream channel for AudioDriverLegacy";
    return res;
  }
...
void AudioDriverLegacy::Cleanup() {
  TRACE_DURATION("audio", "AudioDriverLegacy::Cleanup");
  // TODO(MTWN-385): Figure out a better way to assert this!
  OBTAIN_EXECUTION_DOMAIN_TOKEN(token, &owner_->mix_domain());
...
TimelineFunction AudioDriverLegacy::clock_mono_to_ring_pos_bytes() const {
...
std::optional<Format> AudioDriverLegacy::GetFormat() const {
  TRACE_DURATION("audio", "AudioDriverLegacy::GetFormat");
  std::lock_guard<std::mutex> lock(configured_format_lock_);
  return configured_format_;
}

zx_status_t AudioDriverLegacy::GetDriverInfo() {
  TRACE_DURATION("audio", "AudioDriverLegacy::GetDriverInfo");
  // TODO(MTWN-385): Figure out a better way to assert this!
  OBTAIN_EXECUTION_DOMAIN_TOKEN(token, &owner_->mix_domain());
...
zx_status_t AudioDriverLegacy::Configure(const Format& format,
                                         zx::duration min_ring_buffer_duration) {
  TRACE_DURATION("audio", "AudioDriverLegacy::Configure");
  // TODO(MTWN-385): Figure out a better way to assert this!
  OBTAIN_EXECUTION_DOMAIN_TOKEN(token, &owner_->mix_domain());
...
zx_status_t AudioDriverLegacy::Start() {
  TRACE_DURATION("audio", "AudioDriverLegacy::Start");
  // TODO(MTWN-385): Figure out a better way to assert this!
  OBTAIN_EXECUTION_DOMAIN_TOKEN(token, &owner_->mix_domain());
...
zx_status_t AudioDriverLegacy::Stop() {
  TRACE_DURATION("audio", "AudioDriverLegacy::Stop");
  // TODO(MTWN-385): Figure out a better way to assert this!
  OBTAIN_EXECUTION_DOMAIN_TOKEN(token, &owner_->mix_domain());
...
zx_status_t AudioDriverLegacy::SetPlugDetectEnabled(bool enabled) {
  TRACE_DURATION("audio", "AudioDriverLegacy::SetPlugDetectEnabled");
  // TODO(MTWN-385): Figure out a better way to assert this!
  OBTAIN_EXECUTION_DOMAIN_TOKEN(token, &owner_->mix_domain());
...
zx_status_t AudioDriverLegacy::ReadMessage(const zx::channel& channel, void* buf, uint32_t buf_size,
                                           uint32_t* bytes_read_out, zx::handle* handle_out) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ReadMessage");
...
zx_status_t AudioDriverLegacy::ProcessStreamChannelMessage() {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessStreamChannelMessage");
  zx_status_t res;
...
zx_status_t AudioDriverLegacy::ProcessRingBufferChannelMessage() {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessRingBufferChannelMessage");
  zx_status_t res;
  zx::handle rxed_handle;
  uint32_t bytes_read;
  union {
...
zx_status_t AudioDriverLegacy::ProcessGetStringResponse(audio_stream_cmd_get_string_resp_t& resp) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessGetStringResponse");
...
zx_status_t AudioDriverLegacy::ProcessGetGainResponse(audio_stream_cmd_get_gain_resp_t& resp) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessGetGainResponse");
...
zx_status_t AudioDriverLegacy::ProcessGetFormatsResponse(
    const audio_stream_cmd_get_formats_resp_t& resp) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessGetFormatsResponse");
  if (!fetching_driver_info()) {
...
zx_status_t AudioDriverLegacy::ProcessSetFormatResponse(
    const audio_stream_cmd_set_format_resp_t& resp, zx::channel rb_channel) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessSetFormatResponse");
  ...
  if (res != ZX_OK) {
    FX_PLOGS(ERROR, res) << "Failed to wait on ring buffer channel for AudioDriverLegacy";
    return res;
  }
...
zx_status_t AudioDriverLegacy::ProcessGetClockDomainResponse(
    audio_stream_cmd_get_clock_domain_resp_t& resp) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessGetClockDomainResponse");
...
zx_status_t AudioDriverLegacy::ProcessGetFifoDepthResponse(
    const audio_rb_cmd_get_fifo_depth_resp_t& resp) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessGetFifoDepthResponse");
...
zx_status_t AudioDriverLegacy::ProcessGetBufferResponse(const audio_rb_cmd_get_buffer_resp_t& resp,
                                                        zx::vmo rb_vmo) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessGetBufferResponse");
...
zx_status_t AudioDriverLegacy::ProcessStartResponse(const audio_rb_cmd_start_resp_t& resp) {
...
zx_status_t AudioDriverLegacy::ProcessStopResponse(const audio_rb_cmd_stop_resp_t& resp) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessStopResponse");
...
zx_status_t AudioDriverLegacy::ProcessPositionNotify(const audio_rb_position_notify_t& notify) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ProcessPositionNotify");
...
void AudioDriverLegacy::ShutdownSelf(const char* reason, zx_status_t status) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ShutdownSelf");
...
void AudioDriverLegacy::SetupCommandTimeout() {
  TRACE_DURATION("audio", "AudioDriverLegacy::SetupCommandTimeout");
...
void AudioDriverLegacy::ReportPlugStateChange(bool plugged, zx::time plug_time) {
  TRACE_DURATION("audio", "AudioDriverLegacy::ReportPlugStateChange");
  {
...
zx_status_t AudioDriverLegacy::OnDriverInfoFetched(uint32_t info) {
  TRACE_DURATION("audio", "AudioDriverLegacy::OnDriverInfoFetched");
...
zx_status_t AudioDriverLegacy::SendSetGain(const AudioDeviceSettings::GainState& gain_state,
                                           audio_set_gain_flags_t set_flags) {
  TRACE_DURATION("audio", "AudioDriverLegacy::SendSetGain");
...
zx_status_t AudioDriverLegacy::SelectBestFormat(
...
void AudioDriverLegacy::StreamChannelSignalled(async_dispatcher_t* dispatcher,
                                               async::WaitBase* wait, zx_status_t status,
                                               const zx_packet_signal_t* signal) {
  if (status != ZX_OK) {
...
void AudioDriverLegacy::RingBufferChannelSignalled(async_dispatcher_t* dispatcher,
                                                   async::WaitBase* wait, zx_status_t status,
                                                   const zx_packet_signal_t* signal) {
  if (status != ZX_OK) {
    FX_PLOGS(ERROR, status) << "Async wait failed";
...
void AudioDriverLegacy::DriverCommandTimedOut() {
