...
bool IsSampleFormatInSupported(driver_fidl::SampleFormat sample_format,
                               uint8_t bytes_per_sample,
                               uint8_t valid_bits_per_sample,
                               const driver_fidl::PcmSupportedFormats& supported_formats) {
...
bool IsNumberOfChannelsInSupported(uint32_t number_of_channels,
                                   const driver_fidl::PcmSupportedFormats& format) {
...
bool IsRateInSupported(uint32_t frame_rate, const driver_fidl::PcmSupportedFormats& format) {
...
bool IsFormatInSupported(const fuchsia::media::AudioStreamType& stream_type,
                         const std::vector<driver_fidl::PcmSupportedFormats>& supported_formats) {
...
zx_status_t SelectBestFormat(const std::vector<driver_fidl::PcmSupportedFormats>& fmts,
  ...
  for (const auto& format : fmts) {
    ...
    } else if (IsSampleFormatInSupported(driver_fidl::SampleFormat::PCM_SIGNED, 4, 24, format)) {
      this_sample_format = {driver_fidl::SampleFormat::PCM_SIGNED, 4, 24};
      sample_format_score = 4;
    } else if (IsSampleFormatInSupported(driver_fidl::SampleFormat::PCM_SIGNED, 2, 16, format)) {
      this_sample_format = {driver_fidl::SampleFormat::PCM_SIGNED, 2, 16};
      sample_format_score = 3;
    } else if (IsSampleFormatInSupported(driver_fidl::SampleFormat::PCM_FLOAT, 4, 32, format)) {
      this_sample_format = {driver_fidl::SampleFormat::PCM_FLOAT, 4, 32};
      sample_format_score = 2;
    } else if (IsSampleFormatInSupported(driver_fidl::SampleFormat::PCM_UNSIGNED, 1, 8, format)) {
      this_sample_format = {driver_fidl::SampleFormat::PCM_UNSIGNED, 1, 8};
    ...
    if (IsNumberOfChannelsInSupported(pref_channels, format)) {
      ...
      this_channels = *std::max_element(format.number_of_channels.begin(),
                                        format.number_of_channels.end());
