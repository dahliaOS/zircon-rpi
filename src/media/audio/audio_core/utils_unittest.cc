...
TEST(UtilsFormatTest, SelectBestFormatFoundFidl) {
  std::vector<driver_fidl::PcmSupportedFormats> fmts;
  ...
  formats.sample_formats.push_back(driver_fidl::SampleFormat::PCM_FLOAT);
  ...
  ASSERT_EQ(SelectBestFormat(fmts, &frames_per_second_inout, &channels_inout,
                             &sample_format_inout), ZX_OK);
  ASSERT_EQ(sample_format_inout, fuchsia::media::AudioSampleFormat::FLOAT);
  ASSERT_EQ(frames_per_second_inout, static_cast<uint32_t>(96'000));
  ASSERT_EQ(channels_inout, static_cast<uint32_t>(1));

  // Add a second format range.
  driver_fidl::PcmSupportedFormats formats2 = {};
  ...
  formats2.sample_formats.push_back(driver_fidl::SampleFormat::PCM_SIGNED);
...
TEST(UtilsFormatTest, SelectBestFormatErrorFidl) {
  ...
  ASSERT_EQ(SelectBestFormat(fmts, &frames_per_second_inout, &channels_inout,
                             &sample_format_inout), ZX_ERR_INVALID_ARGS);
  ...
  ASSERT_EQ(SelectBestFormat(fmts, &frames_per_second_inout, nullptr,  // Bad pointer.
                             &sample_format_inout), ZX_ERR_INVALID_ARGS);
  ASSERT_EQ(SelectBestFormat(fmts, &frames_per_second_inout, &channels_inout,
                             nullptr), ZX_ERR_INVALID_ARGS);  // Bad pointer.

  std::vector<driver_fidl::PcmSupportedFormats> empty_fmts;
  ASSERT_EQ(SelectBestFormat(empty_fmts, &frames_per_second_inout, &channels_inout,
                             &sample_format_inout), ZX_ERR_NOT_SUPPORTED);
}
