...
class AudioDriver {
 public:
  ...
  virtual const std::shared_ptr<RingBuffer>& ring_buffer() const FXL_NO_THREAD_SAFETY_ANALYSIS = 0;
  virtual TimelineFunction clock_mono_to_ring_pos_bytes() const FXL_NO_THREAD_SAFETY_ANALYSIS = 0;
};

// TODO(41922): Remove AudioDriverLegacy once the transition to FIDL is completed.
class AudioDriverLegacy : public AudioDriver {
 public:
  // Timeout values are chosen to be generous while still providing some guard-rails against
  // hardware errors. Correctly functioning hardware and drivers should never result in any
  // timeouts.
  ...
  AudioDriverLegacy(AudioDevice* owner);

  using DriverTimeoutHandler = fit::function<void(zx::duration)>;
  AudioDriverLegacy(AudioDevice* owner, DriverTimeoutHandler timeout_handler);

  virtual ~AudioDriverLegacy() = default;
 ...
 private:
  ...
  //
  // Note: Only AudioDriverLegacy writes to these, and only when in our owner's mixing execution
  ...
  const std::shared_ptr<RingBuffer>& ring_buffer() const override FXL_NO_THREAD_SAFETY_ANALYSIS {
    return ring_buffer_;
  };

  TimelineFunction clock_mono_to_ring_pos_bytes() const override FXL_NO_THREAD_SAFETY_ANALYSIS;
...
class AudioDriverFidl : public AudioDriver {
 public:
  AudioDriverFidl(AudioDevice* owner);
 ...
 private:
  ...
  // Accessors for the ring buffer pointer and the current output clock transformation.
  //
  // Note: Only AudioDriverFidl writes to these, and only when in our owner's mixing execution
  // domain.  It is safe for our owner to read these objects, but only when operating in the mixing
  // domain.  Unfortunately, it is not practical to use the static thread safety annotation to prove
  // that we are accessing these variable from the mixing domain.  Instead, we...
  //
  ...
  void RestartWatchPlugState();
  void RestartWatchClockRecovery();
  ...
  const std::vector<fuchsia::hardware::audio::PcmSupportedFormats>& formats() const {
    return formats_;
  }
