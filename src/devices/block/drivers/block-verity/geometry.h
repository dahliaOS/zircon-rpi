...
#ifndef SRC_STORAGE_BLOCK_DRIVERS_BLOCK_VERITY_GEOMETRY_H_
#define SRC_STORAGE_BLOCK_DRIVERS_BLOCK_VERITY_GEOMETRY_H_
...
BlockAllocation BestSplitFor(uint32_t block_size, uint32_t hash_size,
...
struct HashLocation {
  ...
  // The index of the hash within that block.  To get a byte offset, multiply hash_in_block by hash_size.
...
class Geometry {
 public:
  ...
  // block index and intra-block hash offset that covers that inderict block.
...
#endif // SRC_STORAGE_BLOCK_DRIVERS_BLOCK_VERITY_GEOMETRY_H_
