...
struct spn_handle_pool
{
  ...
  struct
  {
    ...
    struct spn_handle_pool_br reclaim[SPN_HANDLE_POOL_RECLAIM_TYPE_COUNT];  // FIXME(allanmac): need to pad
  } wip;
};
...
//
// Globally assume that the _maximum_ push constant limit is 256
// bytes.
//
...
spn_device_handle_pool_reclaim_size(struct spn_vk_target_config const * const config)
{
  ...
  return (push_size_paths - OFFSETOF_MACRO(struct spn_vk_push_paths_reclaim, path_ids)) /
         sizeof(spn_handle_t);
}
...
spn_device_handle_pool_create(struct spn_device * const device, uint32_t const handle_count)
{
  ...
  // NOTE(allanmac): the "block pad" may necessary for this allocator.
  ...
  if (reclaim->rem == 0)
    {
      ...
      spn(device_dispatch_acquire(device, SPN_DISPATCH_STAGE_RECLAIM, &id));
...
spn_result_t
spn_device_handle_pool_validate_retain_d_pathxss(struct spn_device * const     device,
                                                 struct spn_path const * const paths,
                                                 uint32_t const                count)
{
  union spn_paths_to_handles const p2h = { paths };
...
spn_result_t
spn_device_handle_pool_validate_retain_d_rasters(struct spn_device * const       device,
                                                 struct spn_raster const * const rasters,
                                                 uint32_t const                  count)
{
  union spn_rasters_to_handles const r2h = { rasters };
