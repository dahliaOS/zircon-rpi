...
#define SPN_GLSL_MAX(t_,a_,b_)                 (((a_) > (b_)) ? (a_) : (b_))
#define SPN_GLSL_MIN(t_,a_,b_)                 (((a_) < (b_)) ? (a_) : (b_))
...
#if 1
#define SPN_GLSL_EXTRACT_UVEC2_UINT(v_,o_,n_)  (unpackUint2x32(packUint2x32(v_) >> o_)[0] & SPN_GLSL_BITS_TO_MASK(n_))
#else
#define SPN_GLSL_EXTRACT_UVEC2_UINT(v_,o_,n_)  SPN_BITFIELD_EXTRACT(packUint2x32(v_),o_,n_) // might be supported
#endif

#if 1
#define SPN_GLSL_EXTRACT_UVEC2_INT(v_,o_,n_)   SPN_BITFIELD_EXTRACT(unpackInt2x32(packInt2x32(v_) >> o_)[0],0,n_)
#else
#define SPN_GLSL_EXTRACT_UVEC2_INT(v_,o_,n_)   SPN_BITFIELD_EXTRACT(packInt2x32(v_),o_,n_)  // might be supported
#endif
