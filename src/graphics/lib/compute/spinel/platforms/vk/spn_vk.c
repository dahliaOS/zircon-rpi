...
#define SPN_VK_P_EXPAND_X(_p_idx, _p_id, _p_descs) +1
...
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, _ds) +1
...
#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, _ds) SPN_VK_DS_E(_ds_id) = _ds_idx,
...
{
#undef SPN_VK_P_EXPAND_X
#define SPN_VK_P_EXPAND_X(_p_idx, _p_id, _p_descs) VkPipeline _p_id;
...
#undef SPN_VK_P_EXPAND_X
#define SPN_VK_P_EXPAND_X(_p_idx, _p_id, _p_descs) #_p_id,
...
{
#undef SPN_VK_P_EXPAND_X
#define SPN_VK_P_EXPAND_X(_p_idx, _p_id, _p_descs) VkPipelineLayout _p_id;
...
#define SPN_VK_DSLB_NAME(_ds_id) spn_vk_dslb_##_ds_id

#define SPN_VK_DSLB_CREATE(_ds_id, ...)                                                            \
  static const VkDescriptorSetLayoutBinding SPN_VK_DSLB_NAME(_ds_id)[] = { __VA_ARGS__ };

#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, ...) SPN_VK_DSLB_CREATE(_ds_id, __VA_ARGS__)
...
#define SPN_VK_DSLCI_NAME(_ds_id) spn_vk_dslci_##_ds_id

#define SPN_VK_DSLCI_CREATE(_ds_id)                                                                \
  static const VkDescriptorSetLayoutCreateInfo SPN_VK_DSLCI_NAME(                                  \
    _ds_id)[] = { SPN_VK_DSLCI(SPN_VK_DSLB_NAME(_ds_id)) }

#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, _ds) SPN_VK_DSLCI_CREATE(_ds_id);
...
#define SPN_VK_DSL_DECLARE(_ds_id) VkDescriptorSetLayout _ds_id

struct spn_vk_dsl
{
#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, _ds) SPN_VK_DSL_DECLARE(_ds_id);
...
#define SPN_VK_DUTD_NAME(_ds_id) spn_vk_dutd_##_ds_id

#define SPN_VK_DUTD_DEFINE(_ds_id, _ds)                                                            \
  struct SPN_VK_DUTD_NAME(_ds_id)                                                                  \
  {                                                                                                \
    _ds                                                                                            \
  };

#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, _ds) SPN_VK_DUTD_DEFINE(_ds_id, _ds)
...
#define SPN_VK_DUTDP_DEFINE(_ds_id)                                                                \
  struct                                                                                           \
  {                                                                                                \
    struct SPN_VK_DUTD_NAME(_ds_id) * dutds;                                                       \
    uint32_t *        pool;                                                                        \
    VkDescriptorSet * ds;                                                                          \
    uint32_t          rem;                                                                         \
    uint32_t          size;                                                                        \
...
#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, _ds) SPN_VK_DUTDP_DEFINE(_ds_id)
...
#define SPN_VK_DESC_TYPE_STORAGE_BUFFER(_ds_id, _d_idx, _d_ext, _d_id)                             \
  { .dstBinding      = _d_idx,                                                                     \
    ...
    .descriptorCount = 1,                                                                          \
    .descriptorType  = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,                                          \
    .offset          = offsetof(struct SPN_VK_DUTD_NAME(_ds_id), _d_id),                           \
    .stride          = 0 },
...
#define SPN_VK_DUTE_NAME(_ds_id) spn_vk_dute_##_ds_id

#define SPN_VK_DUTE_CREATE(_ds_id, ...)                                                            \
  static const VkDescriptorUpdateTemplateEntry SPN_VK_DUTE_NAME(_ds_id)[] = { __VA_ARGS__ };

#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, ...) SPN_VK_DUTE_CREATE(_ds_id, __VA_ARGS__)
...
#define SPN_VK_DUT_DECLARE(_ds_id) VkDescriptorUpdateTemplate _ds_id

struct spn_vk_dut
{
#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, _ds) SPN_VK_DUT_DECLARE(_ds_id);
...
#define SPN_VK_DP_DECLARE(_ds_id) VkDescriptorPool _ds_id

struct spn_vk_dp
{
#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, _ds) SPN_VK_DP_DECLARE(_ds_id);
...
#undef SPN_VK_DESC_TYPE_STORAGE_BUFFER
#define SPN_VK_DESC_TYPE_STORAGE_BUFFER(_ds_id, _d_idx, _d_ext, _d_id)                             \
  { .type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, .descriptorCount = SPN_VK_DPS_COUNT(_ds_id) },

#undef SPN_VK_DESC_TYPE_STORAGE_IMAGE
#define SPN_VK_DESC_TYPE_STORAGE_IMAGE(_ds_id, _d_idx, _d_ext, _d_id)                              \
  { .type = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, .descriptorCount = SPN_VK_DPS_COUNT(_ds_id) },

#define SPN_VK_DPS_NAME(_ds_id) spn_vk_dps_##_ds_id

#define SPN_VK_DPS_DEFINE(_ds_id, ...)                                                             \
  VkDescriptorPoolSize const SPN_VK_DPS_NAME(_ds_id)[] = { __VA_ARGS__ };
...
#define SPN_VK_HOST_DS(_p_id, _ds_idx, _ds_id) struct SPN_VK_DUTD_NAME(_ds_id) _ds_id;

#undef SPN_VK_HOST_PUSH
#define SPN_VK_HOST_PUSH(_p_id, _p_pc)

#define SPN_VK_PDUTD_NAME(_p_id) spn_vk_pdutd_##_p_id

#define SPN_VK_PDUTD_DEFINE(_p_id, _p_descs)                                                       \
  struct SPN_VK_PDUTD_NAME(_p_id)                                                                  \
  {                                                                                                \
    _p_descs                                                                                       \
  };

#undef SPN_VK_P_EXPAND_X
#define SPN_VK_P_EXPAND_X(_p_idx, _p_id, _p_descs) SPN_VK_PDUTD_DEFINE(_p_id, _p_descs)
...
#define SPN_VK_DPS_COUNT(_ds_id) instance->config.ds._ds_id.sets

#define SPN_VK_DP_CREATE(_ds_idx, _ds_id, ...)                                                     \
  {                                                                                                \
    SPN_VK_DPS_DEFINE(_ds_id, __VA_ARGS__);                                                        \
                                                                                                   \
    dpci.maxSets       = instance->config.ds._ds_id.sets;                                          \
    dpci.poolSizeCount = ARRAY_LENGTH_MACRO(SPN_VK_DPS_NAME(_ds_id));                              \
    dpci.pPoolSizes    = SPN_VK_DPS_NAME(_ds_id);                                                  \
    vk(CreateDescriptorPool(environment->d, &dpci, environment->ac, &instance->dp.named._ds_id));  \
  }

#undef SPN_VK_DS_EXPAND_X
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, ...) SPN_VK_DP_CREATE(_ds_idx, _ds_id, __VA_ARGS__);
...
#define SPN_VK_DS_EXPAND_X(_ds_idx, _ds_id, _ds)                                                    \
  ...
  //
  bool const p_ok[SPN_VK_P_COUNT] = { [16] = true };
  ...
  //
#undef SPN_VK_P_EXPAND_X
#define SPN_VK_P_EXPAND_X(...) true,
  bool const p_ok[SPN_VK_P_COUNT] = { SPN_VK_P_EXPAND() };
...
#undef SPN_VK_HOST_DS
#define SPN_VK_HOST_DS(_p_id, _ds_idx, _ds_id) instance->dsl.named._ds_id,

#define SPN_VK_PL_CREATE(_p_idx, _p_id, ...)                                                       \
  {                                                                                                \
...
#undef SPN_VK_P_EXPAND_X
#define SPN_VK_P_EXPAND_X(_p_idx, _p_id, ...) SPN_VK_PL_CREATE(_p_idx, _p_id, __VA_ARGS__)
  ...
  for (uint32_t ii = 0; ii < SPN_VK_P_COUNT; ii++)
    {
      ...
      fprintf(stdout, "%-38s ", SPN_VK_PIPELINE_NAMES[ii]);
...
#define SPN_VK_HOST_DS(_p_id, _ds_idx, _ds_id)                                                     \
  SPN_VK_DS_BIND_PROTO(_p_id, _ds_id)                                                              \
  {                                                                                                \
