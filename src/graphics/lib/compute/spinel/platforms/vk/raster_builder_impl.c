...
static void
spn_rbi_path_append(struct spn_raster_builder_impl * const impl, spn_path_t const * const path)
{
...
struct spn_rbi_complete_payload_2
{
  ...
  struct
  {
    ...
    VkDeviceSize    offset;  // required for transfer
  } temp;
};
...
spn_rbi_complete_1(void * pfn_payload)
{
  ...
  payload_2->temp.offset  = payload_1->temp.offset;
  ...
  // DS: TTRKS -- rebind
  //
  spn_vk_ds_bind_rasters_alloc_ttrks(instance, cb, payload_2->ds.t);
  ...
  *spn_vk_ds_get_raster_ids_raster_ids(instance, payload_2->ds.i) = impl->vk.rings.rc;
  ...
  //
  // FIXME(allanmac): the target needs to encode the raster_alloc
  // workgroup size
  //
  uint32_t const subgroup_size_log2 = impl->config->subgroup_size_log2;
  uint32_t const subgroup_size_ru   = (1u << subgroup_size_log2) - 1u;
  uint32_t const rasters_alloc_x    = (dispatch->rc.span + subgroup_size_ru) >> subgroup_size_log2;

  // dispatch one subgroup (workgroup) per raster
  vkCmdDispatch(cb, rasters_alloc_x, 1, 1);

  // compute barrier
  vk_barrier_compute_w_to_compute_r(cb);

  printf("dispatch->rc.span = { %u, %u }\n", dispatch->rc.span, rasters_alloc_x);
                   ...
                   uint32_t * const                       bc_count)
{
  ...
  uint32_t           sh_lo      = MIN_MACRO(uint32_t, sh->head + sh->span, sh_size) - sh->head;
  VkBufferCopy *     bc_lo      = bcs + (*bc_count)++;
  ...
  if (sh->span > sh_lo)
    {
      uint32_t       sh_hi = sh->span - sh_lo;
      VkBufferCopy * bc_lo = bcs + (*bc_count)++;

      bc_lo->srcOffset = ring_base_h;
      bc_lo->dstOffset = ring_base_d;
      bc_lo->size      = elem_size * sh_hi;
    }
}
...
spn_rbi_flush(struct spn_raster_builder_impl * const impl)
{
  ...
  *spn_vk_ds_get_rasterize_fill_quads(instance, payload_1->ds.r) = impl->vk.rings.tc;
  ...
  // for now we need to save the offset -- we won't after this is
  // replaced with a compute shader
  payload_1->temp.offset = dbi_ttrks->offset;
  ...
  if (impl->config->raster_builder.vk.rings.d != 0)
    {
      ...
      uint32_t const sh_size = impl->mapped.cf.ring.size;
  ...
  ////////////////////////////////////////////////////////////////
  //
  // INITIALIZE: ZERO RASTERIZE & TTRKS
  //
  ////////////////////////////////////////////////////////////////
  ...
  VkDeviceSize const ttrks_offset = dbi_ttrks->offset + OFFSETOF_MACRO(struct spn_rc_meta, blocks);
  VkDeviceSize const ttrks_size   = SPN_VK_BUFFER_OFFSETOF(ttrks, ttrks, ttrks_count) +
                                  SPN_VK_BUFFER_MEMBER_SIZE(ttrks, ttrks, ttrks_count) -
                                  OFFSETOF_MACRO(struct spn_rc_meta, blocks);

  vkCmdFillBuffer(cb, dbi_ttrks->buffer, ttrks_offset, ttrks_size, 0);

  ////////////////////////////////////////////////////////////////
  //
  // BARRIER FOR BOTH THE DISCRETE-GPU COPIES AND FILLS
  //
  ////////////////////////////////////////////////////////////////
  ...
  uint32_t const cmds_per_wg = impl->config->raster_builder.fills_scan.rows
                               << impl->config->subgroup_size_log2;
...
#define SPN_PATH_BUILDER_PRIM_TYPE_EXPAND_X(_p, _i, _n)                                            \
  ...
  VkBufferCopy const bc = { .srcOffset = payload_1->temp.offset +
                                         SPN_VK_BUFFER_OFFSETOF(ttrks, ttrks, ttrks_count),
                            .dstOffset = sizeof(*impl->mapped.cb.extent) * payload_1->dispatch_idx,
                            .size      = sizeof(*impl->mapped.cb.extent) };

  vkCmdCopyBuffer(cb, dbi_ttrks->buffer, impl->vk.copyback.dbi.buffer, 1, &bc);
            ...
            uint32_t                               count)
{
  ...
  //
  // validate and retain the paths before we proceed
  //
  spn_result_t result;

  result = spn_device_handle_pool_validate_retain_h_paths(impl->device, paths, count);
