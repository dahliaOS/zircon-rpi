...
use {
    ...
    fuchsia_inspect::vmo as finspect,
...
struct SnoopConfig {
    ...
    // Inspect tree
    _config_inspect: finspect::Node,
    _log_size_bytes_metric: finspect::UintProperty,
    _log_time_metric: finspect::UintProperty,
    _max_device_count_metric: finspect::UintProperty,
    _truncate_payload_property: finspect::StringProperty,
    _hci_dir_property: finspect::StringProperty,
}

impl SnoopConfig {
    /// Creates a strongly typed `SnoopConfig` out of primitives parsed from the command line
    fn from_opt(opt: Opt, config_inspect: finspect::Node) -> SnoopConfig {
...
fn run(
    ...
    mut service_handler: impl Unpin + FusedStream + Stream<Item = ServiceRequests>,
            ...
            select! {
                ...
                request_stream = service_handler.select_next_some() => {
                    match request_stream {
                        ServiceRequests::Snoop(stream) => {
                            register_new_client(stream, &mut client_requests, id_gen.next())
                        }
                    }
                }
...
fn main() {
    ...
    let inspector = finspect::Inspector::new().expect("Can't create Inspector VMO");
    if let Err(e) = inspector.export(&mut fs) {
        fx_log_warn!("Could not export Inspect VMO to service fs: {:?}", e);
    };
    ...
    fs.dir("public").add_fidl_service(ServiceRequests::Snoop);
