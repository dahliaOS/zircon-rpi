...
const auto kTestUnauthenticatedLinkKeyType =
    hci::LinkKeyType::kUnauthenticatedCombination192;
...
class NoOpPairingDelegate final : public PairingDelegate {
 public:
  ...
  fxl::WeakPtr<NoOpPairingDelegate> GetWeakPtr() {
  ...
  void DisplayPasskey(PeerId peer_id, uint32_t passkey, bool local_consent,
...
TEST(GAP_PairingStateTest,
     PairingStateRejectsPairingInitiationWithoutPairingDelegate) {
  auto connection = MakeFakeConnection();
  bool owner_cb_called = false;
  auto owner_status_cb = [&owner_cb_called](auto, hci::Status status) {
    owner_cb_called = true;
  };
  PairingState pairing_state(kTestPeerId, &connection,
                             std::move(owner_status_cb));

  bool initiator_cb_called = false;
  auto initiator_status_cb = [&initiator_cb_called](auto, hci::Status status) {
    initiator_cb_called = true;
  };
  EXPECT_EQ(PairingState::InitiatorAction::kDoNotSendAuthenticationRequest,
            pairing_state.InitiatePairing(std::move(initiator_status_cb)));
  EXPECT_FALSE(pairing_state.initiator());
  EXPECT_FALSE(owner_cb_called);
  EXPECT_TRUE(initiator_cb_called);
}

TEST(GAP_PairingStateTest,
     PairingStateRejectsPairingResponseWithoutPairingDelegate) {
  auto connection = MakeFakeConnection();
  bool cb_called = false;
  auto status_cb = [&cb_called](auto, hci::Status status) { cb_called = true; };
  PairingState pairing_state(kTestPeerId, &connection, std::move(status_cb));
...
TEST(GAP_PairingStateTest, SuccessfulEncryptionChangeTriggersStatusCallback) {
  ...
  pairing_state.OnLinkKeyNotification(kTestLinkKeyValue,
                                      kTestUnauthenticatedLinkKeyType);
...
TEST(GAP_PairingStateTest, EncryptionChangeToEnableCallsInitiatorCallbacks) {
  ...
  static_cast<void>(
      pairing_state.InitiatePairing(status_handler_0.MakeStatusCallback()));
  static_cast<void>(pairing_state.OnIoCapabilityRequest());
  pairing_state.OnIoCapabilityResponse(kTestPeerIoCap);
  pairing_state.OnUserConfirmationRequest(kTestPasskey,
                                          NoOpUserConfirmationCallback);
  ...
  static_cast<void>(
      pairing_state.InitiatePairing(status_handler_1.MakeStatusCallback()));
...
TEST(GAP_PairingStateTest,
     NumericComparisonPairingDisplaysPasskeyToInitiatorDisplayYesNoSide) {
  TestStatusHandler status_handler;
  auto connection = MakeFakeConnection();
  PairingState pairing_state(kTestPeerId, &connection,
                             status_handler.MakeStatusCallback());
  ...
  pairing_delegate.SetCompletePairingCallback(
      [](PeerId peer_id, sm::Status status) {
        EXPECT_EQ(kTestPeerId, peer_id);
        EXPECT_TRUE(status);
      });
  pairing_state.OnSimplePairingComplete(hci::StatusCode::kSuccess);
...
TEST(GAP_PairingStateTest,
     PasskeyEntryPairingDisplaysPasskeyToDisplayOnlySide) {
  TestStatusHandler status_handler;
  auto connection = MakeFakeConnection();
  PairingState pairing_state(kTestPeerId, &connection,
                             status_handler.MakeStatusCallback());
...
TEST(GAP_PairingStateTest,
  ...
  ASSERT_FALSE(pairing_state.initiator());
  EXPECT_EQ(IOCapability::kKeyboardOnly,
            *pairing_state.OnIoCapabilityRequest());
...
TEST(GAP_PairingStateTest, JustWorksPairingDoesNotRequestUserAction) {
  ...
  pairing_state.OnIoCapabilityResponse(IOCapability::kNoInputNoOutput);
  ASSERT_FALSE(pairing_state.initiator());
  EXPECT_EQ(IOCapability::kNoInputNoOutput,
            *pairing_state.OnIoCapabilityRequest());
...
TEST_P(HandlesEvent,
       InIdleStateAfterIoCapRequestRejectedWithoutPairingDelegate) {
...
TEST_P(HandlesEvent, InFailedStateAfterPairingFailed) {
  ...
  pairing_state().OnSimplePairingComplete(
      hci::StatusCode::kAuthenticationFailure);
  EXPECT_EQ(1, status_handler().call_count());
  ASSERT_TRUE(status_handler().status());
