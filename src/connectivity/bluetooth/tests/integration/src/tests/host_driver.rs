...
use crate::harness::host_driver::{
    expect_adapter_state, expect_eq, expect_host_peer, expect_no_host_peer, expect_remote_device, HostDriverHarness,
};
...
pub async fn test_forget(test_state: HostDriverHarness) -> Result<(), Error> {
    ...
    let connected = peer::identifier(&le_peer.identifier).and(peer::connected(true));
    await!(expect_host_peer(&test_state, connected))?;
    ...
    let forgotten = peer::identifier(&le_peer.identifier);
    await!(expect_no_host_peer(&test_state, forgotten))?;
