...
impl Peer {
    ...
    pub fn start_stream(
        ...
        async move {
            ...
            {
                // Start the media decoding task with this call to set_started().
                let strong_peer = peer.upgrade().ok_or(avdtp::Error::PeerDisconnected)?;
                let mut strong_peer = strong_peer.lock();
                strong_peer.set_started(&local_id).map_err(|e| avdtp::Error::RequestInvalid(e))
            }
        }
    }
...
impl PeerInner {
    ...
    fn set_started(&mut self, seid: &avdtp::StreamEndpointId) -> Result<(), avdtp::ErrorCode> {
...
mod tests {
    ...
    use crate::connected_peers::SBC_SEID;
