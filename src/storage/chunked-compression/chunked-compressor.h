...
class ChunkedCompressor {
 public:
  ...
  static Status CompressBytes(const void* data, size_t len,
  ...
  // |dst_len| must be at least |ComputeOutputSizeLimit(len)| bytes long.
  // Returns the number of compressed bytes written in |bytes_written_out|.
  Status Compress(const void* data, size_t len, void* dst, size_t dst_len,
