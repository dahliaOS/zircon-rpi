...
zx_status_t IsBoardName(const char* requested_board_name) {
  ...
  if (strncmp(board_name, requested_board_name, strlen(requested_board_name)) == 0) {
      return ZX_OK;
  }
  printf("Board name: %s.\n", board_name);
  return kWrongBoard;
...
void IspTest::SetUp() {
  ...
  // does allow waiting on folders from devmgr.
...
int main(int argc, char** argv) {
  ...
  if (status == kWrongBoard) {
      printf("Not the correct board. skipping tests.\n");
      return 0;
  }
  return status;
}
